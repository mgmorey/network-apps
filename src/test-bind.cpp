...
auto main(int argc, char* argv[]) -> int
{
    ...
    try {
        ...
        test_bind_invalid_addr(invalid_addr, inet_hints);
        const Endpoint invalid_host {".", localservice};
        test_bind_invalid_host(invalid_host, unspec_hints);
        const Endpoint invalid_service {localhost, "."};
        test_bind_invalid_service(invalid_service, unspec_hints);
        const Endpoint valid_endpoint {
            args.size() > 1 ? args[1] : localhost,
            args.size() > 2 ? args[2] : localservice
        };
        test_bind_valid(valid_endpoint, unspec_hints);
    }
